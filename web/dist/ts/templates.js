...
class CustomDropdown extends HTMLElement {
    constructor() {
        ...
        const selElmnt = node.querySelector("select");
        ...
        const selElmntTag = selElmnt.children[0];
        ...
        for (let j = 1; j < selElmnt.length; j++) {
            ...
            c.addEventListener("click", function () {
                ...
                for (let k = 0; k < y.length; k++) {
                    ...
                    const yl = this.parentNode?.querySelector(".same-as-selected");
                    if (yl) {
...
document.addEventListener("click", function () {
    closeAllSelect(this.activeElement);
});
function closeAllSelect(elmnt) {
    const arrNo = [];
    const selectItemsElements = document.getElementsByClassName("select-items");
    const selectSelectedElements = document.getElementsByClassName("select-selected");
